// === SECURITY: Ensure user is authenticated instantly ===
...
async function saveProfile() {
    ...
    const isPhone = /^[0-9+\-\s()]{7,20}$/.test(number);
    ...
    if (!isPhone && !isEmail) {
        alert("Contact must be a valid mobile number or email.");
        return;
    }
...
function logout() {
    // Insta-lock: Clear auth data immediately
    ApiClient.clearAuthToken();
    localStorage.removeItem("user");
    localStorage.removeItem("loggedIn");
